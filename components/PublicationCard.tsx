...
function AuthorList({ authors }: { authors: string }) {
  return (
    <>
      {authors.split(/(Junli Zhao|赵俊莉)/gi).map((part, index) =>
        /^(Junli Zhao|赵俊莉)$/i.test(part) ? <strong key={`${part}-${index}`} className="font-bold text-slate-900">{part}</strong> : part,
      )}
    </>
  );
}
...
export function PublicationCard({ publication, compact = false }: { publication: Publication; compact?: boolean }) {
  ...
  return (
    <article className={`group border-t border-slate-200 py-6 ${compact ? "first:border-t-0" : "sm:grid sm:grid-cols-[72px_1fr] sm:gap-6"}`}>
      ...
      <div className={publication.image ? "grid gap-5 md:grid-cols-[210px_1fr] md:items-start" : ""}>
        {publication.image ? (
          <div className="relative aspect-[16/9] overflow-hidden border border-slate-200 bg-[#092a49]">
            <Image src={publication.image} alt={`${publication.title} 研究主题示意图`} fill sizes="(max-width: 768px) 100vw, 210px" className="object-cover" />
          </div>
        ) : null}
        <div>
